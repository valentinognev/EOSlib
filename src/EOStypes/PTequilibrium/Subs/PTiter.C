...
{   // lambda1 > lambda1_max
    ...
    for( count=1; count<max_iter1; count++)
    {
        ...
        if( NotDomain1(V1,e1) )
            break;    
        if( std::abs(P1-P2)<tol*P1+tol_P && std::abs(T1-T2)<tol*T1+tol_T )
            return 0;
    }
    if( std::abs(P1-P2)<0.1*P1 && std::abs(T1-T2)<0.1*T1 )
    {
        return PT0iter(V,e);
    }
    if( verbose )
...
{   // lambda2 > lambda2_max
    ...
    for( count=1; count<max_iter1; count++)
    {
        ...
        if( NotDomain2(V2,e2) )
            break;    
        if( std::abs(P1-P2)<tol*P2+tol_P && std::abs(T1-T2)<tol*T2+tol_T )
            return 0;
    }
    if( std::abs(P1-P2)<0.1*P2 && std::abs(T1-T2)<0.1*T2 )
    {
        return PT0iter(V,e);
    }
    if( verbose )
